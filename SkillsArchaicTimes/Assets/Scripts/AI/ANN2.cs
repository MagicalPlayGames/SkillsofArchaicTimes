...
public class ANN2 : MonoBehaviour
{
    ...
    //Constructor 1
    public void setNetwork(byte layers, int[] nodes, byte divisbleBy, int iterations, float learningRate, byte algoNum, byte outputs)
    {
    ...
    public void setNetwork(ANNParams input)
    {
        ...
        control = new brain();
        this.control.output.nodes = new node[input.outputs];
        ...
        for (int i = 0; i < input.layers; i++)
        {
            ...
            for (int j = 0; j < input.nodes[i]; j++)
            {
                ...
                newestNode.curSum = newestNode.output = 0;
                if (i == input.layers - 1)
                    newestNode.currentWeights = new float[control.output.nodes.Length];
                else
    ...
    //Overall Processing
    public bool processData(string inputs, float[] expectedOutcome)
    {
        readInput(inputs);
        writeOutputs();
    ...
    public void readTrueOutput(float[] output)
    {
        control.realOutput = new layer();
        control.realOutput.nodes = new node[output.Length];
        for (int i = 0; i < output.Length; i++)
        {
            control.realOutput.nodes[i].output = output[i];
        }
    }
